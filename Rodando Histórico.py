...
'''Importantando base com o histórico já rodada (se houver), para não ter que rodar de novo'''


Hist = pd.read_csv(
    r'C:\Users\femdi\OneDrive\Documentos\Python\PyCharm\Carteira_de_Acoes\Rentabilidades.csv',
    error_bad_lines=False)

Lista_Datas_hist = Hist['Lista_Datas']

Dias_ja_Rodados = len(Lista_Datas_hist)
...
for v in tqdm(Lista_Datas[Dias_ja_Rodados:]):
...
'''Criando um DataFrame com as Rentabilidade e as Datas'''
Rentabilidades = pd.DataFrame()
Rentabilidades0 = Rentabilidades.assign(Lista_Datas=Lista_Datas[len(Lista_Datas_hist):], Lista_de_Rentabilidade=Lista_de_Rentabilidade)

'''Juntando com o Histórico'''
Rentabilidades1 = pd.concat([Hist,Rentabilidades0])
Rentabilidades2 = Rentabilidades1.reset_index().drop(columns = {'index'})


'''Salvando no PC como novo Histórico'''
Rentabilidades2.to_csv(r'C:\Users\femdi\OneDrive\Documentos\Python\PyCharm\Carteira_de_Acoes\Rentabilidades.csv', index = False)




Historico_Completo = pd.read_csv(
    r'C:\Users\femdi\OneDrive\Documentos\Python\PyCharm\Carteira_de_Acoes\Rentabilidades.csv',
    error_bad_lines=False)
...
import matplotlib.pyplot as plt
ibov=[]
for i in np.arange(len(Serie_IBOV)):
         x=(Serie_IBOV.iloc[i]['IBOV']-Serie_IBOV.iloc[0]['IBOV'])/Serie_IBOV.iloc[0]['IBOV']
         ibov=np.append(ibov,x)
ibov1=Serie_IBOV.assign(Variação= ibov)

ibov2=ibov1.reset_index(level=0).rename(columns={"index": "Date"})


ibov3=ibov2.assign(Lista_de_Rentabilidade=Historico_Completo['Lista_de_Rentabilidade'])
print(ibov3)

plt.figure(figsize=(16,6))
plt.plot(ibov3['Date'], ibov3['Variação'])
plt.plot(ibov3['Date'], ibov3['Lista_de_Rentabilidade'])
...
'''Fazendo uma matriz de Covariância entre as ações'''
...
prices0 = pd.DataFrame()
tickers = PapeisSA
data = Lista_Datas[len(Lista_Datas)-1]
data2 = Lista_Datas[0]

'''Caso há algum erro na base da Yahoo, o valor que retorna é uma média da cotação desde o primeiro dia da carteira'''
for i in tickers:
        prices0[i] = round(web.get_data_yahoo(i, data2)['Adj Close'], 2)
prices = prices0.T

sns.heatmap(prices.corr(), annot=True, vmin=-1, vmax=1)
plt.show()
